...
// mono, <=1275 bytes. No duration/allocation controlled by the incoming packet.
...
static inline OSStatus PLANKMicOpusProvide(AudioConverterRef converter, UInt32 *packets,
    ...
    data->mBuffers[0] = (AudioBuffer){1, input->size, (void *)input->data};
...
static inline bool PLANKMicDecoderCreate(PLANKMicDecoder *decoder) {
    ...
    input.mFramesPerPacket = 480; input.mChannelsPerFrame = 1;
    output.mSampleRate = 48000; output.mFormatID = kAudioFormatLinearPCM;
    output.mFormatFlags = kAudioFormatFlagsNativeFloatPacked;
    output.mFramesPerPacket = 1; output.mChannelsPerFrame = 1; output.mBitsPerChannel = 32;
    output.mBytesPerFrame = output.mBytesPerPacket = sizeof(float);
...
static inline bool PLANKMicDecode(PLANKMicDecoder *decoder, const uint8_t *data, size_t size, float samples[480]) {
    ...
    output.mBuffers[0] = (AudioBuffer){1, 480 * sizeof(float), samples};
    UInt32 frames = 480;
    ...
    if ((status && status != -7777) || !input.supplied || !frames || frames > 480 ||
        (!decoder->priming && frames != 480) || output.mBuffers[0].mDataByteSize != frames*sizeof(float)) return false;
    ...
    if (frames < 480) {
        memmove(samples + 480 - frames, samples, frames*sizeof(float));
        memset(samples, 0, (480-frames)*sizeof(float));
    }
    decoder->priming = false;
    for (unsigned i = 0; i < 480; i++) if (!isfinite(samples[i])) return false;
    return true;
}
